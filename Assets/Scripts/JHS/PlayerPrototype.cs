...
public enum ScaleSize { low, middle, high };
...
public class PlayerPrototype : MonoBehaviour
{
    ...
    public ScaleSize size = ScaleSize.middle;
    // Start is called once before the first execution of Update after the MonoBehaviour is created
    void Awake()
    {
        rb = GetComponent<Rigidbody>();
        size = ScaleSize.middle;
    ...
    {
        if (size == ScaleSize.high) return;
        size = (ScaleSize)((int)size + 1);
        transform.localScale = transform.localScale * 4;
    }
    private void DecreaseScale()
    {
        if (size == ScaleSize.low) return;
        size = (ScaleSize)((int)size - 1);
        transform.localScale = transform.localScale / 4;
    }
